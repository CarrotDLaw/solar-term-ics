...
def parse_hko(pageurl):
    ...
    sql_jq = "insert or replace into ical (date,lunardate,jieqi) " "values(?,?,?) "
    conn = sqlite3.connect(DB_FILE)
    db = conn.cursor()
    for line in lines:
        m = RE_CAL.match(line)
...
def gen_cal(start, end, fp):
    ...
    if startyear > 1900 and endyear < 2101:
        ...
        sql = (
            "select date, lunardate, holiday, jieqi from ical "
    ...
    for r in rows:
        ...
        if r["jieqi"]:
            ld.append(r["jieqi"])
        uid = "%s-carrotdlaw@gmail.com" % r["date"]
        summary = " ".join(ld)
...
def gen_cal_jieqi_only(start, end, fp):
    ...
    for r in rows:
        ...
        #     ld.append(r['holiday'])
        if r["jieqi"]:
            ld.append(solar_term_dict[r["jieqi"]])
        uid = "%s-carrotdlaw@gmail.com" % r["date"]
        summary = " ".join(ld)
        utcstamp = datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")
        dt = datetime.strptime(r["date"], "%Y-%m-%d")
...
def lunaryear(isodate):
    ...
    sql = (
        'select date from ical where lunardate="正月" and '
...
def main():
    ...
    helpmsg = (
        "Usage: lunar_ical.py --start=startdate --end=enddate --jieqi\n"
        "Example: \n"
    ...
    try:
        opts, args = getopt.getopt(
            sys.argv[1:], "h", ["start=", "end=", "help", "jieqi"]
        )
    ...
    while start < 2101:
        ...
        for i in range(len(aalc)):
            ...
            if aaday != hkoday or aaldate != hkoldate:
                print("AA %s %s, HKO %s %s" % (aaday, aaldate, hkoday, hkoldate))
        start += 1
